...
def test_flat_seam_is_identity():
    ...
    corr = [{
        "vert": True, "od": 200.0, "dy": 100, "dx": 0,
        "cy": np.array([0.0, 0.05]), "cx": np.array([0.0, 0.0]),  # < _MIN_CORRECTION_PX
        "sign": +0.5, "side": "i",
    }]
...
def test_folded_kernel_matches_warp_then_blend():
    ...
    warped = np.stack([map_coordinates(tile[c], [yy + Dy, xx + Dx], order=1, mode="nearest")
                       for c in range(C)]).astype(np.float32)
    f_old = np.zeros((C, subY, subX), np.float32); w_old = np.zeros((C, subY, subX), np.float32)
    ...
    f_new = np.zeros((C, subY, subX), np.float32); w_new = np.zeros((C, subY, subX), np.float32)
    accumulate_tile_shard_distorted(f_new, w_new, tile, w2d, 0, 0,
                                    sy0, sx0, subY, subX, 0.0, 0.0, Dy, Dx)
